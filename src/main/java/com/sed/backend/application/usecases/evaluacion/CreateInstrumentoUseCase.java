...
public class CreateInstrumentoUseCase {

    private final InstrumentoServiceImpl instrumentoService;
    private final PeriodoServiceImpl periodoService;

    @Transactional
    public InstrumentoResponse execute(InstrumentoCreateRequest request) {
        Periodo periodo = buscarPeriodo(request.getPeriodoId());
        ...
        Instrumento guardado = instrumentoService.guardar(instrumento);
        return InstrumentoMapper.toResponse(guardado);
    }

    private Periodo buscarPeriodo(UUID periodoId) {
        if (periodoId == null) {
            throw new IllegalArgumentException("Periodo requerido para crear un instrumento");
        }
        return periodoService.buscarPorId(periodoId);
    }
}
